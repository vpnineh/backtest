...
ACCOUNT = dict(
    initial_bal  = 5_000.0,
    risk_pct     = 0.005,     # 0.5% Risk for safety
    max_open     = 1,         
    daily_dd_lim = 0.03,      
    max_dd_kill  = 0.07,      
)
...
COMMISSION_PIP = 0.6          # $6 per lot round-trip
SLIPPAGE_PIP = 0.2            
...
SWEEP_CFG = dict(
    ...
    min_range_pip = 10,       
    max_range_pip = 50,       
    atr_sl_mult   = 1.5,      # 1.5x ATR safety buffer
    macro_ema     = 50,       # D1 EMA period for trend alignment
)
...
def strategy_prop_killer(m1: pd.DataFrame, pair: str) -> tuple:
    ...
    d1["ema_macro"] = d1["ema_macro"].shift(1) # Prevent lookahead!
