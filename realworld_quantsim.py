"""
CorrArb v9i — Apply Regime Filters
=====================================
فیلترهای جدید بر اساس آنالیز v9h:
  ✅ z_entry MAX = 2.3  (نه بیشتر — Z بزرگ ضرر می‌دهد)
  ✅ vr_max = 0.75      (نه 0.90)
  ✅ حذف ساعت‌های زیان‌ده: 4,5,7,9,13,18,20
"""
...
class Config:
    ...
    z_fast_period      = 96
    z_entry            = 2.1
    z_entry_max        = 2.3    # ← جدید: Z نباید از این بیشتر باشد
    z_exit_partial     = 0.50
    z_exit_full        = 0.0
    z_stop_margin      = 4.0
    ...
    hour_start         = 2
    hour_end           = 19
    bad_hours          = {4, 5, 7, 9, 13, 18, 20}  # ← جدید
    ...
    vr_period          = 200
    vr_k               = 4
    vr_max             = 0.75   # ← جدید: از 0.90 به 0.75
...
def compute_signals(df, label=""):
    ...
    regime_ok = vr < C.vr_max   # ← فیلتر VR اعمال می‌شود
    ...
    # فیلتر ساعت — بدون ساعت‌های زیان‌ده
    ...
    # فیلتر Z — فقط بین z_entry و z_entry_max
    long_ok  = (z < -C.z_entry) & (z > -C.z_entry_max)
    short_ok = (z >  C.z_entry) & (z <  C.z_entry_max)
    ...
    n = int((sig != 0).sum())
    print(f"  {label}Signals: {n:,} | VR<{C.vr_max} | Z:[{C.z_entry},{C.z_entry_max}]")
    return sig, z
...
def run_backtest(df, sig, z, verbose=True):
    ...
    for bar in range(C.warmup, len(idx)):
        ...
        if pos is not None:
            cp = c_[bar]; d = pos['dir']
            ep = pos['entry']; zn = zz_[bar]; lr = pos['lot_rem']
                ...
                if ((d==1 and zn>=-C.z_exit_partial) or
                    ...
                    if p_lot >= C.min_lot:
                        ...
                        if p_pnl > 0:
                            ...
                            all_trades.append(tr); acc['trades'].append(tr)
...
def print_report(res, label):
    ...
    print(f"  MonthlyAvg: ${monthly.mean():.2f}  Median: ${monthly.median():.2f}")
    ...
    for yr, g2 in df.groupby('year'):
        ...
        print(f"    {mark} {yr}: {len(g2):>4}T  WR:{len(w2)/len(g2)*100:5.1f}%  "
...
if __name__ == "__main__":
    t0 = datetime.now()
    print("╔══════════════════════════════════════════════════════╗")
    print("║   CorrArb v9i — Regime Filters Applied              ║")
    print("╚══════════════════════════════════════════════════════╝")

    df = load_audnzd()

    # ── Baseline (v9g) ──
    print("\n▶ Baseline (VR<0.90, all hours, all Z)")
    Config.vr_max      = 0.90
    Config.z_entry_max = 99.0   # بدون محدودیت
    Config.bad_hours   = set()  # همه ساعات
    sig0, z0 = compute_signals(df, "Baseline ")
    res0 = run_backtest(df, sig0, z0, verbose=False)
    print_report(res0, "BASELINE — VR<0.90 | All Hours | All Z")

    # ── فقط VR سخت‌تر ──
    print("\n▶ Filter 1: فقط VR<0.75")
    Config.vr_max      = 0.75
    Config.z_entry_max = 99.0
    Config.bad_hours   = set()
    sig1, z1 = compute_signals(df, "VR<0.75  ")
    res1 = run_backtest(df, sig1, z1, verbose=False)
    print_report(res1, "FILTER 1 — VR<0.75 only")

    # ── فقط Z محدود ──
    print("\n▶ Filter 2: فقط Z:[2.1, 2.3]")
    Config.vr_max      = 0.90
    Config.z_entry_max = 2.3
    Config.bad_hours   = set()
    sig2, z2 = compute_signals(df, "Z<2.3    ")
    res2 = run_backtest(df, sig2, z2, verbose=False)
    print_report(res2, "FILTER 2 — Z max=2.3 only")

    # ── فقط ساعت بهتر ──
    print("\n▶ Filter 3: فقط حذف ساعت‌های بد")
    Config.vr_max      = 0.90
    Config.z_entry_max = 99.0
    Config.bad_hours   = {4, 5, 7, 9, 13, 18, 20}
    sig3, z3 = compute_signals(df, "BadHour  ")
    res3 = run_backtest(df, sig3, z3, verbose=False)
    print_report(res3, "FILTER 3 — Bad Hours removed")

    # ── همه فیلترها با هم ──
    print("\n▶ Filter ALL: VR<0.75 + Z<2.3 + No Bad Hours")
    Config.vr_max      = 0.75
    Config.z_entry_max = 2.3
    Config.bad_hours   = {4, 5, 7, 9, 13, 18, 20}
    sig4, z4 = compute_signals(df, "ALL      ")
    res4 = run_backtest(df, sig4, z4, verbose=True)
    print_report(res4, "FILTER ALL — VR<0.75 + Z<2.3 + No Bad Hours")

    # ── جدول مقایسه ──
    print("\n" + "╔" + "═"*66 + "╗")
    print("║" + "  SUMMARY".center(66) + "║")
    print("╚" + "═"*66 + "╝")
    print(f" {'Config':<30}  {'PF':>5}  {'Bank':>8}  {'Net':>8}  {'+Mo':>4}  {'Pass':>4}  {'Blow':>4}")
    print(" " + "─"*66)

    for label, res in [
        ("Baseline (VR<0.90)", res0),
        ("VR<0.75 only",       res1),
        ("Z<2.3 only",         res2),
        ("No Bad Hours only",  res3),
        ("ALL filters",        res4),
    ]:
        ...
        pos_m  = int((monthly > 0).sum())
        tot_m  = len(monthly)
        print(f" {label:<30}  {pf:>5.3f}  "
              f"{res['withdrawn']:>8,.0f}  "
              f"{dft['pnl'].sum():>8,.0f}  "
              f"{pos_m:>4}  {n_pass:>4}  {n_blow:>4}")
