"""
CorrArb v13 — Auto-Calibration Engine (Quad Portfolio)
======================================================
AUDNZD + AUDCAD + GBPCAD + GBPCHF
Includes automated 'best_z' correlation filter and Grid Search Optimizer.
"""
...
def run_portfolio(pair_data, corr_mode='best_z'):
    G = GlobalConfig
    cidx = None
    ...
    for bar in range(G.warmup, N):
        ...
        if corr_mode == 'best_z' and candidates:
...
# REPORT
# ═══════════════════════════════════════════════════════
def print_report(res, title, suffix):
    ...
    ci = res['common_idx']
    all_months = pd.period_range(start=ci[GlobalConfig.warmup].to_period('M'), end=ci[-1].to_period('M'), freq='M')
    ...
    return {
        'Mode': suffix, 'Trades': len(df), 'WR%': round(wr, 1), 'PF': round(pf, 2),
        'Banked': round(res['withdrawn']), 'MonAvg': round(monthly.mean()), 'Blown': n_blow
    }


# ═══════════════════════════════════════════════════════
# AUTO-OPTIMIZER (GRID SEARCH ENGINE)
# ═══════════════════════════════════════════════════════
def run_calibration(raw_data_dict):
    param_grid = {
        'z_entry': [1.9, 2.1, 2.3],         
        'z_exit_partial': [0.4, 0.5, 0.6],  
        'partial_ratio': [0.60, 0.75]       
    }
    ...
    print(f"\n  ⚙️ Starting Auto-Calibration for 'best_z' mode...")
    print(f"  🔍 Testing {total_runs} combinations (This may take 30-40 minutes)...\n")
    ...
    for i, combo in enumerate(combinations, 1):
        ...
        new_pair_data = {}
        for name, (df, pcfg) in raw_data_dict.items():
            sig, z = compute_signals(df, pcfg)
            new_pair_data[name] = (df, sig, z, pcfg)

        old_stdout = sys.stdout
        sys.stdout = open(os.devnull, 'w')
        try:
            res = run_portfolio(new_pair_data, corr_mode='best_z')
            report = print_report(res, "Opt", "opt")
        finally:
        ...
        if report:
            ...
            print(f"  [{i:02d}/{total_runs}] Tested: z_entry={combo[0]}, z_exit={combo[1]}, p_ratio={combo[2]} "
                  f"--> MonAvg: ${report['MonAvg']}, Blown: {report['Blown']}")

    df_res = pd.DataFrame(results)
    df_res = df_res.sort_values(by=['Blown', 'MonAvg'], ascending=[True, False])

    print("\n" + "═"*85)
    print("  🏆 TOP 5 CALIBRATED SETTINGS (Optimized for Safety & Yield)")
    print("═"*85)
    cols = ['z_entry', 'z_exit_partial', 'partial_ratio', 'Trades', 'WR%', 'PF', 'Banked', 'MonAvg', 'Blown']
    print(df_res[cols].head(5).to_string(index=False))
    print("═"*85)
    
    df_res.to_csv("calibration_full_results.csv", index=False)
    print("\n  📊 Full grid saved to 'calibration_full_results.csv'")
...
if __name__ == "__main__":
    t0 = datetime.now()
    print("╔══════════════════════════════════════════════════════════════╗")
    print("║  CorrArb v13 — Auto-Calibration Engine                       ║")
    print("╚══════════════════════════════════════════════════════════════╝")
    ...
    for name, pcfg in PAIR_CFG.items():
        df = build_pair(pcfg)
        if df is not None:
            raw_data_dict[name] = (df, pcfg)

    print(f"\n  ✅ Raw Data Loaded in {(datetime.now()-t0).total_seconds():.1f}s")
    
    run_calibration(raw_data_dict)
    
    print(f"\n  ✅ Total Process Completed in {(datetime.now()-t0).total_seconds():.1f}s")
