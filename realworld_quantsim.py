"""
CorrArb v16 — Sweet Spot
==========================
هدف: MonthAvg $65-80 با Blown ≤4

تغییرات نسبت به v15b:
  1. max_trades_day = 2 برگشت (ولی GBP limit حفظ شد)
  2. risk_pct به v13 برگشت — dd_levels خودش scale می‌کند
  3. VR fast حذف شد — فقط VR slow باقی ماند
  4. spread stress و swap واقع‌بینانه v15b حفظ شد
  5. GBP exposure limit حفظ شد
  6. Risk Control v14b حفظ شد
"""
...
class GlobalConfig:
    ...
    max_trades_day = 2              # v16: برگشت به 2 (ولی GBP limit حفظ شد)
    ...
    vr_period = 200                 # v16: فقط VR slow — fast حذف شد
    vr_k      = 4
    corr_period = 96

    # stress spread: فقط اول باز شدن session
    stress_hours = {0, 7, 12}
    slippage_normal = 0.7
    slippage_stress = 1.5
...
PAIR_CFG = {
    'AUDNZD': {
        ...
        'spread_pip': 2.5,
        'spread_pip_stress': 5.0,
        'swap_pips_day': -0.15,
        'pip_size': 0.0001,
        'vr_max': 0.75,             # v16: برگشت به v13
        'corr_min': 0.82,           # v16: کمی بالاتر از v13 (0.80) برای جبران حذف VR fast
        'risk_pct': 0.015,          # v16: برگشت به v13
        'risk_min': 0.005,
        'sl_pips': 30.0,
        'tp_pips': 90.0,
    },
    'AUDCAD': {
        ...
        'pip_size': 0.0001,
        'vr_max': 0.85,
        'corr_min': 0.57,           # v16: کمی بالاتر از v13 (0.55)
        'risk_pct': 0.010,          # v16: برگشت به v13
        'risk_min': 0.004,
        'sl_pips': 25.0,
        'tp_pips': 75.0,
    },
    'GBPCAD': {
        ...
        'formula': 'mul', 'quote': 'inv_leg2',
        'spread_pip': 4.0,
        'spread_pip_stress': 9.0,
        'swap_pips_day': -0.40,
        'pip_size': 0.0001,
        'vr_max': 0.88,
        'corr_min': 0.47,           # v16: کمی بالاتر از v13 (0.45)
        'risk_pct': 0.008,          # v16: برگشت به v13
        'risk_min': 0.003,
        'sl_pips': 28.0,
        'tp_pips': 84.0,
        'gbp_pair': True,
    ...
    'GBPCHF': {
        ...
        'pip_size': 0.0001,
        'vr_max': 0.90,
        'corr_min': 0.42,           # v16: کمی بالاتر از v13 (0.40)
        'risk_pct': 0.007,          # v16: برگشت به v13
        'risk_min': 0.003,
        'sl_pips': 35.0,
        'tp_pips': 105.0,
        'gbp_pair': True,
...
def run_portfolio(pair_data):
    ...
    for bar in range(G.warmup, N):
        ...
        # GBP exposure limit — حداکثر 1 GBP pair هم‌زمان
        gbp_open = sum(
            1 for n in pair_data
            if positions[n] is not None and pair_data[n][3].get('gbp_pair', False)
        )
        ...
        total_float = sum(
             ...
             * p['lot_rem'] * G.lot_size * pa[n]['qr'][bar]
...
def print_report(res, title):
    ...
    monthly.to_csv('monthly_v16.csv', header=['pnl'])
    pd.DataFrame(res['eq_curve']).to_csv('equity_v16.csv', index=False)
    print("  📊 monthly_v16.csv + equity_v16.csv saved")

    print("\n  📈 تکامل نسخه‌ها:")
    rows = [
        ("v13  بدون RiskCtrl",  91, 59, 3, 10, "—"),
        ("v14b با RiskCtrl",    68, 56, 3,  2, "0.94"),
        ("v15b Balanced",       29, 54, 4,  3, "0.64"),
        (f"v16  Sweet Spot",
...
if __name__ == "__main__":
    t0 = datetime.now()
    print("╔══════════════════════════════════════════════════════════════╗")
    print("║  CorrArb v16 — Sweet Spot                                  ║")
    print("╚══════════════════════════════════════════════════════════════╝")
    print()
    print("  تغییرات نسبت به v15b:")
    print("  • max_trades_day: 1→2 (ولی GBP limit حفظ شد)")
    print("  • risk_pct: برگشت به v13 — dd_levels خودش scale می‌کند")
    print("  • VR fast حذف شد — فقط VR slow")
    print("  • corr_min کمی بالاتر (جبران حذف VR fast)")
    print("  • spread stress + swap واقع‌بینانه حفظ شد")
    print()
    ...
    print_report(res, "v16 — Quad Sweet Spot (AUDNZD+AUDCAD+GBPCAD+GBPCHF)")
