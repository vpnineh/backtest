"""
CorrArb Prop Simulator — v7 Multi-Pair + Regime Filter + Partial Exit
======================================================================
جفت‌ارزها : EURGBP synthetic (EURUSD/GBPUSD)
            AUDNZD synthetic (AUDUSD/NZDUSD)
بهبودها   : Variance Ratio Regime Filter
            Partial Exit (50% at z=0.8, rest at z=0.3)
            SL = 30 pips  (↓ از 40)
            Risk = 1.5%   (↑ از 1%)
            min_profit = $20 (↑ از $10)
            TimeStop = 48 bars (↓ از 72)
            Dynamic lot sizing با quote rate واقعی
...
class Config:
    ...
    z_entry            = 2.1
    z_exit_partial     = 0.8         # خروج 50% اول
    z_exit_full        = 0.3         # خروج باقی‌مانده
    z_stop_margin      = 4.0         # کات رژیم
    min_net_profit_usd = 20.0        # ↑ $10 → $20
    ...
    sl_pips            = 30.0        # ↓ 40 → 30
    tp_pips            = 90.0
    time_stop_bars     = 48          # ↓ 72 → 48  (12 ساعت)
    ...
    vr_k               = 4           # نسبت دوره
    vr_max             = 0.95        # VR < 0.95 → mean-reverting ✓
...
def run_backtest(pairs: dict, pair_signals: dict) -> dict:
    ...
    print(f"\n  ▶ Running Multi-Pair Strict Prop Simulator...")
...
def print_report(results: dict):
    ...
    print("\n" + "═" * 70)
    print(f" ▌  CorrArb Prop Simulator v7 — {'+'.join(pair_names)}  ▐")
    print("═" * 70)
