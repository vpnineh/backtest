...
class MartingaleManager:
    ...
    def maybe_add(self, basket: Basket, bid_price: float, ask_price: float, current_time: datetime,
        ...
        if basket.direction == "BUY":
            adverse_move = last.entry_price - bid_price
            exec_price = ask_price + slip_price
        else:
            adverse_move = ask_price - last.entry_price
        ...
        if adverse_move >= grid_distance_price:
            ...
            basket.add_position(exec_price, new_lot, current_time, last.level + 1, "martingale", commission)
            return True
        return False
